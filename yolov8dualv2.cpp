...
void camera_thread_func(const std::string cam_dev, const std::string cam_name,
                        int target_size = 416, float conf_thresh = 0.80f)
{
    try {
...
int main()
{
    std::string cam0 = "/dev/video10";
    std::string name0 = "cam1";
    std::string cam1 = "/dev/video12";
    std::string name1 = "cam2";

    stop_all = false;

    std::thread t0(camera_thread_func, cam0, name0, 416, 0.80f);
    std::thread t1(camera_thread_func, cam1, name1, 416, 0.80f);
